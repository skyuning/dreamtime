...
const nucleus = {
  ...
  async init() {
    ...
    try {
      ...
      this.instance = Nucleus(this.getAppId(), config)

      // Get global application settings
      this.settings = (await axios.get(
...
module.exports = new Proxy(nucleus, {
  get(obj, prop) {
    ...
    if (prop in obj.settings) {
      return obj.settings[prop]
    }

    if (obj.instance && prop in obj.instance) {
      if (obj.isEnabled) {
        return obj.instance[prop]
      }
    }
